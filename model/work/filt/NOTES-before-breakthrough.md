...
## Slot filter (tests/filt_probe, filt_id, filt_id2, filt_coef, filt_imp, filt_reset, filt_cyc; tools/filt*; HANDOVER.md)

- **Structure: two-pole state-variable low-pass**, output inverted:
  `band += f*x - f*low - f*q*band;  low += f*band;  out = -low` — equivalently y = f²x + (2 - f² - fq) y1 -
  (1 - fq) y2.  Least-squares coefficients from captures equal the formulas below to ~1e-6.
- **Cutoff (log-style):** e = v >> 9, **f = (256 + v[8:1]) * 2^(e - 24)** where v is the 13-bit FEG value (FLV
  scale).  FLV bit 0 is unused (0x1E00 ≡ 0x1E01, 0x1F55 gives 852/853 of nominal).  Verified for e = 12..15 over
  the whole mantissa; e = 15 with v[8:1] = 0xFF reads f = 1.0 (not 511/512): all four 0x1FFE step responses are
  consistent only with k = 512 there (first output 8000 for x = 8000; k = 511 would give 7968).  Mechanism unknown.  e = 11 fits were biased
  (deadband), lower exponents unverified.  FLV ≈ 0x1FF8 makes the filter unstable (it saturates).
- **Resonance (log-style):** Q' = Q + 4, **q = (16 - Q'[2:0]) << (4 - Q'[4:3]) / 128** — exact for all 32 values:
  1.5, 1.375, 1.25, 1.125, 1, 15/16, ... , 0.5, ... , 13/128.  f*q is an exact product (not a log-domain sum).
- **Output precision:** 1/8 sample (MIXS always even with VOFF = 1).
- **State is never cleared**: not by key-on, key-off, or LPOFF (LPOFF = 1 bypasses the filter and freezes its
  state; clearing LPOFF resumes from the old state).  With truncation the state can stick: at FLV 0x1400 the
  output stays at 7.75 samples forever with zero input (deadband |f (x - low)| < 1/8).  A VOFF slot's frozen filter
  state is visible at MIXS even while the slot is off.
- **Arithmetic: not yet bit-exact** — rounding study as of 2026-09-23; how to reproduce each point: HANDOVER.md.
  Data: the 24 filt_cyc step responses (rest -> 256 samples of 8A -> zeros, 6648..6710 samples each; y = -MIXS/2 =
  low; tools/filt_step.py -> work/filt/step.bin).  Method: tools/filt_rule.cpp tracks, with exact integers, the SET of
  band values consistent with the capture under a per-product rounding rule or a rule family (low is known: it is the
  output); tools/filt_need.cpp reads the forced roundings off the capture where the band is pinned.
  - **low and band are both in 1/8-sample units** (band possibly 1/4 at e = 15; not separated).  low has no hidden
    bits: any hidden low bits make every deterministic rule fit worse.  A band finer than 1/8 is refuted at e = 15
    (streams 0x1FF0 Q 0, 0x1F55), 1/2 sample too; lower cutoffs cannot discriminate.
  - **The band update is ONE product: `band += R(f * (x - low - q*band))`** (q*band exact in the tests so far).
    With any one-LSB rounding of it and of the low update all 24 streams fit end to end (k = 256, 426, 504, 510,
    512; e = 12..15; Q 0 and 4; both signs).  Rounding f*x, f*low and f*q*band (or f*(x - low) and f*q*band)
    separately fits only 0x1E00 and the A = 7 stream under every deterministic rule tried (floor, ceil, toward
    zero, away, half-even, floor(v + n/8), toward zero + n/8, radix-4 Booth arrays truncated at relative or
    absolute columns with either operand recoded) — that is why the old model was exact only at f = 1/2.
  - **That product floored** (low update still free within one LSB) fits 22/24 streams; only e = 15 Q 0 (0x1FFE,
    0x1FF0) fails, after 6-7 samples (q = 1.5: q*band has a 1/2 fraction — rounding before the multiply?).
  - **The low update `low += R(f * band')` is still open.**  With the band update floored, for k = 256 it is
    floor(v) + 1 when frac(v) >= 2 LSB of its own grid (1/8 at f = 1/8, 1/16 at f = 1/16, ...), floor(v) when v is
    exact, and EITHER at exactly one LSB.  For k != 256 it is not a function of the fraction (0x1F55: 426/512 ->
    down, 188/512 -> up, 332/512 -> down): operand-dependent, i.e. a multiplier-array effect to be fitted from the
    pinned operands (filt_need -v).  Equivalent view (band offset): band update ceil + low update floor, except
    that an exact f*band' sometimes comes out one lower (13/24 streams exact under that family); no single state
    feature decides those cases.  No deterministic single-product rule fits more than 2/24 streams yet (the
    separate-product forms reach 5/24, all of them 0x1E00 or the A = 7 stream).
  - Exact (unrounded) arithmetic explains only ~6-24 samples from the best start state and needs 0.5-4 LSB of
    slack for a whole response (tools/filt_lp.py): the rounding is systematic.
  - Sign symmetry y(-A) = ~y(A) holds for 97-100 % of samples at Q 4 (all 6710 at 0x1E00 and 0x1C00; 0x1800
    6524), not universally — the +A and -A runs start from different inherited rest states.  0x1C00 Q 0 is mostly
    y(-A) = -y(A).  Not a law.
  - Rest states before key-on are inherited from the slot's previous batch and can be limit cycles: (0, -1, -1)
    at 0x1FFE/0x1FF0/0x1FFC Q 4 (0x1FF0/0x1FFC settle to fixed points after the step, 0x1FFE keeps cycling),
    (-3, 1, -2, 1, -3, 2) at 0x1FFE Q 0.  Zero-input end states: +2/-3 (0x1C00 Q 4), +1/-2 (0x1A00), 0/-1 (0x1800,
    0x1E00, 0x1FF0 for +A/-A).
  Earlier rejected (tools/filt_search*.cpp, superseded): direct forms, float-stored band (PACK/UNPACK), log-domain
  multiplies (fitted gains exact to 1e-5), "exponent-scaled band precision" (an artefact of partial fits).  The model
  (src/aica_model.cpp lpf_step) still has the old floor/nearest variant (tests/filt_id2: APPROX).
- minicast has no filter at all.
...
- Filter: the low-update rounding (and e = 15 Q 0 / q*band rounding) — see Slot filter and HANDOVER.md; then
  implement in lpf_step; low FLV exponents; FLV top end (f = 1.0 mechanism, other exponents); filter + VOFF=0 precision.
