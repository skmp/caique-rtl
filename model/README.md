...
| `hw/` | console back-end (`io_kos.c`, KOS build) |
| `host/` | model back-end (`io_model.cpp`: G2 access cost 2.4 us per access, sample stepping) |
| `tests/<case>/hw/` | console results (text, captures `*.hdr`/`*.bin`) |
| `tests/<case>/model/` | the model's results for the same case |
| `tools/` | analysis and comparison (python + a few C++ searches) |
| `work/` | scratch (derived data for searches) |
...
## Running
    ...
    # C++ filter builds and validation: see HANDOVER.md. Do not use the legacy Python tools.
