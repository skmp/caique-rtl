/* filt_id.c -- filter identification captures.  Per slot: settle (key-on at FLV 0x1800 Q 0 on a looped silence,
 * 100 ms), key-off, then key-on at the test (F, Q) playing a one-shot [64 zeros, impulse 0x2000, 1023 zeros,
...
static const uint16_t Fv[] = {0x0800, 0x0C00, 0x1000, 0x1200, 0x1400, 0x1600, 0x1800, 0x1A00, 0x1C00, 0x1E00, 0x1F00, 0x1F80};
static const int Qv[] = {0, 4, 8, 16, 24, 31};

int test_main(void) {
    out_open("filt_id.txt");
    ...
    int nf = sizeof Fv / sizeof Fv[0], nq = sizeof Qv / sizeof Qv[0], nc = nf * nq;
    for (int base = 0; base < nc; base += NS) {
        ...
        for (int k = 0; k < NS; k++) {
            ...
            for (int i = 0; i < 5; i++) c.FLV[i] = 0x1800;
