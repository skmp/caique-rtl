/* sub_sched.c -- sub-sample (TODO 1.6): the per-frame schedule of key events, the envelope pass and CPU MIXS accesses.
 *
...
int test_main(void) {
    ...
    for (int k = 0; k < 64; k++) {
        for (int xb = 0; xb < 6; xb++) {
            const int x = xb < 4 ? xb / 2 : xb - 2, batch = xb < 4 ? xb & 1 : 0, nev = x < 2 ? NEV01 : NEV23;
            setup(k, x);
            if (x != 0) ch_keyon(k);
            spin_us(2000);
